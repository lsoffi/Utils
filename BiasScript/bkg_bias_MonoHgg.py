...
from combine_maker_MonoHgg import CombineApp
    ...
    def throwToys(self,options,args):
        ...
        for comp,model in zip(options.components,options.models):
            ...
            for cat in fit["categories"]:
                ...
                treename = "mctruth_all_%s_%s" % (fitname,cat) #mctruth_all
                
                print treename
                
                dset = self.rooData(treename, "weight")
                dset.Print()
                ...
                if options.throw_from_model:
                  ...
                  #  canv.cd(1)
                    ROOT.gPad.SetPad(0.,0.,1.,1.)#0.,0.35,1,1
                    ROOT.gPad.SetLogy()
                    #ROOT.gPad.SetLogx()
...
#                    frame.GetXaxis().SetMoreLogLabels()
                    ...
                    frame.GetYaxis().SetRangeUser( 1.e-3,500. )
                    frame.Draw()
                    ...
                    #resid.GetYaxis().SetRangeUser( -4., 4. )
                    #resid.Draw()
        ...
        for comp,model in zip(options.components,options.models):
            ...
            for cat in categories:
                ...
                for testRange in testRanges:
                    ...
                    biases[rname] = ntp
                    print ntp.GetEntries()," <--- nentries tree"
                    self.store_[ntp.GetName()] = ntp
                    
                generator = self.rooPdf("pdf_mctruth_all_%s_%s" % (fitname,cat))#mctruth_all
                print comp,fitname,cat
                gnorm     = self.buildRooVar("norm_mctruth_all_%s_%s" % (fitname,cat), [], recycle=True)#mctruth_all
                gnorm.Print() 
...
#                    print toy,toyname,self.rooData(toyname).numEntries()
                    ...
                    if options.plot_toys_fits:
                        ...
                        self.plotBkgFitBias(options,dset,pdf,roobs,toyname,slabel,bias_funcs=options.bias_param)
        ...
        for fname,label in zip(options.bias_files,options.bias_labels):
            ...
            for key in ROOT.TIter(fin.GetListOfKeys()):
                ...
                if name.startswith("tree_bias"):                    
                    ...
                    tree.Draw("bias>>h_bias_%s(20,-4.005,4.005)" % nlabel, "bias!=0")
